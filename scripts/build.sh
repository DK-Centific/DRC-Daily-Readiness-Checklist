#!/usr/bin/env bash
# Copy the static site into dist/. If DRC_FLOW_URL is set, write dist/config.local.js
...
cp "$root/index.html" "$root/config.js" "$dist/"
...
if [[ -n "$flow" ]]; then
  ...
  printf 'window.DRC_CONFIG = { backend: '\''pa'\'', FLOW_URL: %s };\n' "$flow_json" > "$dist/config.local.js"
fi
